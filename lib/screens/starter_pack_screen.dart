      ...
      children: [
        ...
        Positioned.fill(
          child: GameLoadingOverlay(
            visible: _opening,
            title: 'OPENING PACK',
            messages: GameLoadingOverlay.packMessages,
          ),
        ),
      ],
    );
        ...
        child: ListView(
          ...
          children: [
            if (_players.isEmpty) ...[
              ...
              Text(
                '15 cards: 1 premium star pull and 14 starter squad players.',
                textAlign: TextAlign.center,
                style: TextStyle(
...
class _GenerationStatus extends StatelessWidget {
  const _GenerationStatus({required this.alreadyOpened});

  final bool alreadyOpened;

  @override
  Widget build(BuildContext context) {
    final existing = alreadyOpened;
    final label =
        existing
            ? 'Starter pack was already opened; showing saved squad cards'
            : 'Starter squad added to your club';
    final accent = existing ? Colors.cyanAccent : GameColors.neon;
    return Container(
